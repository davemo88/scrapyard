...
#include "ScrapyardUIHelpers.h"
#include "Net/UnrealNetwork.h"
#include "Online.h"

FText ScrapyardUIHelpers::GetProfileOpenText() const
{
...
bool ScrapyardUIHelpers::ProfileOpenedUI(const FUniqueNetId& Requestor, const FUniqueNetId& Requestee, const FOnProfileUIClosedDelegate* Delegate) const
{
...
FText ScrapyardUIHelpers::GetProfileSwapText() const
{
	return NSLOCTEXT("Network", "PCSwapProfile", "Space Switch User");
}

bool ScrapyardUIHelpers::ProfileSwapUI(const int ControllerIndex, bool bShowOnlineOnly, const FOnLoginUIClosedDelegate* Delegate) const
{
	const auto OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
